...
async function resolveSubstackProfilePostUrl(urlString) {
  try {
    ...
    const idMatch = parsed.pathname.match(/^\/@[^/]+\/p-(\d+)/);
    if (!idMatch) return urlString;
    const postId = idMatch[1];
    const apiRes = await fetch(
      `https://substack.com/api/v1/posts/by-id/${postId}`,
      substackFetchInit({ Accept: 'application/json, text/plain, */*' })
    );
    if (apiRes.ok) {
      const data = await apiRes.json();
      const canonical = data?.post?.canonical_url;
      if (canonical) {
        try {
